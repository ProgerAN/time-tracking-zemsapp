        ...
        @forelse($projects as $project)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                <th scope="row"
                    ...
                    @if($editedId !== $project->id)
                        <span wire:click="editName({{$project->id}})" class="cursor-pointer">{{ $project->name }}</span>
                    @else

                        <div class="relative">
                            <input wire:keydown.enter="saveName({{$project->id}})" wire:model="name" type="text" id="name" class="block w-full p-4  text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="text . . . " required>
                            <button wire:click="saveName({{$project->id}})" type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">save</button>
                        </div>
